    ...
    def gradient_decompose(self, args=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                self.state[p]['pro_m'] = self.state[p]['g_0'] + abs(-1.0) * self.state[p]['g_2']
                p.grad.data = 1.0 * self.state[p][
                    "g_1"] + 1.0 * p.grad.data.detach().clone()
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                p.grad.data.add_(vertical, alpha=-0.03)
        ...
        def get_grad():
            ...
            with torch.enable_grad():
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    loss, loss_items = self.loss_fn(outputs, targets, **kwargs)
                    if self.RANK != -1:
                        loss *= self.WORLD_SIZE  # gradient averaged between devices in DDP mode
                    if self.opt.quad:
                        loss *= 4.
            loss.backward() ########## !!!!!!!!一定要用这个才行
