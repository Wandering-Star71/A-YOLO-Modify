...
class CoordAtt(nn.Module):
    def __init__(self, inp, ca_ratio):
        super(CoordAtt, self).__init__()
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        mip = _make_divisible(inp * ca_ratio, 4)
...
class SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.sc = SCConv(c_, c_, 1 ,1, 1 ,1, 4,nn.Upsample)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y1 = self.m(x)
            y2 = self.m(y1)
            # print('y1 and y2: ', y1.shape)
            return self.cv2(torch.cat((self.sc(x), y1, y2, self.m(y2)), 1))
...
class GhostConvV2(nn.Module):
    def __init__(self, c1, c2, kernel_size=1, stride=1, ratio=2, dw_size=3, relu=True,mode='original'):
...
class GhostBottleneckV2(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.conv = nn.Sequential(
            GhostConvV2(c1, c_, 1, 1, relu=True, mode='attn'),  # pw
            # DWConv(c_, c_, k, s, act=False) if s == 2 else nn.Identity(),  # dw
            GhostConvV2(c_, c2, 1, 1, relu=False))  # pw-linear
